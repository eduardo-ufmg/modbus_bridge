...
void setup_config_webpage(AsyncWebServer* server, Configs<typename OppositeSerial<SI>::Type, SI>* configs, SI* dbg_serial)
{
	...
	server->on("/configure", HTTP_POST, [configs, dbg_serial](AsyncWebServerRequest* request) {
		...
		dbg_serial->printf("Configs updated| RTU Baudrate: %d, RTU Serial Config: %s\r\n",
			configs->get_rtu_baudrate(), serial_dbg_config_str.at(
				get_uni_serial_config_for_printing(configs->get_rtu_serial_config())
			).c_str());
...
String config_rtu_html(Configs<RTU, DBG>* configs)
{
	...
	for (const auto& baudrate : serial_baudrates) {
		...
		if (baudrate == configs->get_rtu_baudrate()) {
			html += " selected";
		}
		html += ">" + String(baudrate) + "</option>";
	}
	html += "</select>";
	html += "<p>Valor atual: " + String(configs->get_rtu_baudrate()) + "</p>";
	...
	for (const auto& [config, description] : webpage_config_str) {
		...
		if (config == get_uni_serial_config_for_printing(configs->get_rtu_serial_config())) {
			html += " selected";
		}
...
template void setup_config_webpage<SoftwareSerial>(AsyncWebServer* server, Configs<HardwareSerial, SoftwareSerial>* configs, SoftwareSerial* dbg_serial);
template void setup_config_webpage<HardwareSerial>(AsyncWebServer* server, Configs<SoftwareSerial, HardwareSerial>* configs, HardwareSerial* dbg_serial);
